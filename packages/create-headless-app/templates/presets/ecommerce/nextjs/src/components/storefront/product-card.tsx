...
export function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <div className="group rounded-lg border border-border bg-card overflow-hidden transition-shadow hover:shadow-lg">
      <Link href={`/products/${product.id}`} className="block aspect-square bg-muted relative overflow-hidden">
        ...
        {!product.inStock && (
          <div className="absolute top-2 right-2 rounded-full bg-destructive px-2 py-1 text-xs text-white">
            Out of Stock
          </div>
        )}
      </Link>
