...
export function AdminReviewsTable() {
  ...
  const handleBulkAction = async (action: 'approve' | 'reject') => {
    ...
    try {
      ...
      if (action === 'approve') {
        await bulkApproveReviewsFn({ data: { ids } })
      } else {
        await bulkRejectReviewsFn({ data: { ids } })
      }
      fetchReviews()
    } catch {
      // silently handled
  ...
  const handleSheetSave = async () => {
    ...
    setSheetSaving(true)
    try {
      await moderateReviewFn({ data: { id: sheetReview.id, status: sheetStatus } })
      setSheetReview(null)
      fetchReviews()
    } catch {
      // silently handled
                    ...
                    <div className="flex items-center gap-1">
                      {review.status !== 'approved' && (
                        <Button
                          ...
                          onClick={async () => {
                            await moderateReviewFn({ data: { id: review.id, status: 'approved' } })
                            fetchReviews()
                          }}
                          title="Approve"
                        >
                      ...
                      {review.status !== 'rejected' && (
                        <Button
                          ...
                          onClick={async () => {
                            await moderateReviewFn({ data: { id: review.id, status: 'rejected' } })
                            fetchReviews()
                          }}
                          title="Reject"
                        >
