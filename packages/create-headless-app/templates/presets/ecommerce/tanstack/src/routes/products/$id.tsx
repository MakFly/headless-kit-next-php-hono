    ...
    return (
      <div className="min-h-screen bg-background">
        ...
        <div className="mx-auto max-w-4xl px-6 py-16 text-center">
          ...
          <Link to="/products" className="mt-4 inline-flex items-center text-primary hover:underline">
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-6xl px-6 py-8">
        ...
        <div className="grid gap-8 md:grid-cols-2">
          <div className="aspect-square rounded-lg bg-muted flex items-center justify-center text-8xl">{emoji}</div>
          <div>
            <h1 className="text-3xl font-bold">{product.name}</h1>
            <div className="mt-2 flex items-center gap-2">
              <div className="flex items-center">
                {Array.from({ length: 5 }).map((_, i) => (
                  <StarIcon key={i} className={`h-4 w-4 ${i < Math.floor(product.rating) ? 'fill-yellow-400 text-yellow-400' : 'text-muted-foreground/30'}`} />
                ))}
              </div>
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                  {variant.options.map((option) => (
                    <button key={option} onClick={() => setSelectedVariants((prev) => ({ ...prev, [variant.name]: option }))}
                      className={`inline-flex h-9 items-center rounded-md px-3 text-sm font-medium border transition-colors ${selectedVariants[variant.name] === option ? 'border-primary bg-primary text-primary-foreground' : 'border-input hover:bg-accent'}`}>
                      {option}
                    </button>
                  ))}
                </div>
            ...
            <div className="mt-6 flex items-center gap-4">
              ...
              <button onClick={() => { addItem(product, quantity, selectedVariants); toast.success(`${product.name} added to cart`) }}
                ...
                <ShoppingCartIcon className="h-4 w-4" />{product.inStock ? 'Add to Cart' : 'Out of Stock'}
              </button>
            </div>
          </div>
        </div>
