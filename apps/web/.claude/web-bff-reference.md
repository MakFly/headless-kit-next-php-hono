# Next.js BFF — reference (imported from `CLAUDE.md`)
...
Available in `.claude/skills/` (each skill has a `references/` folder — follow `@…` links in `SKILL.md` to load detail + source pointers):
...
- `next-section` — Scaffold a new route group section
- `next-store` — Create a Zustand store with hydration
