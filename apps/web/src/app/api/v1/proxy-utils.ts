 ...
 * Extracted for testability — no Next.js or framework imports.
 */
...
const RATE_LIMIT_MAX_AUTH = 20; // max auth requests per IP per window
...
export function checkRateLimit(ip: string, path: string): boolean {
  ...
  if (entry.count >= RATE_LIMIT_MAX_AUTH) {
    return false;
  }
